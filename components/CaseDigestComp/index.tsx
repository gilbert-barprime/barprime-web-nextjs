...
type PropsType = {
  case_list: CaseDigest[] | null;
};

export default function CaseDigestComp(props: PropsType) {
  const { case_list } = props;
  return (
    <>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {case_list &&
          case_list.map((ele, idx) => (
            <div
              key={idx}
            ...
            >
              <div className="p-4 sm:p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-xs sm:text-sm text-gray-600 mb-2">
                      Justice:{" "}
                      <span className="font-medium">{ele.justice}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="flex space-x-2">
                  <Link
                    href={`/dashboard/case-digest/viewer/${ele._id}`}
